...
export function CardItem({ card, onClick, topicName, topicColor, onContextMenu, onMention, compact }: CardItemProps) {
  ...
  if (compact) {
    return (
      <CardHoverPreview card={card} topicName={topicName} topicColor={topicColor}>
        <div className="group relative mb-2 w-full break-inside-avoid">
          ...
          >
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              {card.is_temp && (
                <span
                  className="shrink-0 inline-block h-1.5 w-1.5 rounded-full bg-amber-400"
                  title={t("temporary")}
                  aria-label={t("temporary")}
                />
              )}
            </div>
            <div className="mt-1 line-clamp-2 [&_*]:!text-[11px] [&_*]:!leading-relaxed [&_*]:!text-text-secondary [&_a]:!pointer-events-none [&_img]:!hidden [&_pre]:!hidden [&_table]:!hidden">
