...
class TestVectorSearch:
    ...
    async def test_limit_respected(self, mock_db):
        ...
        # Path A: 2 chunk rows, Path B: 1 fallback row → 3 total, limit=2 → return 2
        mock_db.execute.side_effect = [
            _chunk_result([
                _make_chunk_row(cards[0].id, 0.1),
                _make_chunk_row(cards[1].id, 0.2),
            ]),
            _card_scalars_result([cards[0], cards[1]]),
            _fallback_result([(cards[2], 0.3)]),
        ]
